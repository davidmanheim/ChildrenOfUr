...
class Processor implements Manager {
  ...
  void addRouteWrapper(Type metadataType, RouteWrapper wrapper,
    ...
    _findHandlers(_getRoutes, metadataType, includeGroups).forEach((m) =>
        (m.handler as RequestTargetMetadata).wrappers
            .add(new RouteWrapperMetadata(wrapper, m.metadata)));
  }
    ...
    handlerTypes.forEach((handlerType) {
      ...
      switch (handlerType) {
        ...
        case HandlerType.interceptor:
          f = _getInterceptors;
          break;
        case HandlerType.errorHandler:
          f = _getErrorHandlers;
          break;
      }

      _findHandlersByParams(f, metadataType).forEach((m) =>
          (m.handler as RequestTargetMetadata).parameterProviders[metadataType] = parameterProvider);
    });
  }
