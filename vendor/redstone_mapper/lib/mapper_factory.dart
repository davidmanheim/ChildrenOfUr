...
final Map<Type, _DynamicMapper> _cache = {
  String: new _DynamicMapper.notEncodable(),
  int: new _DynamicMapper.notEncodable(),
  double: new _DynamicMapper.notEncodable(),
  num: new _DynamicMapper.notEncodable(),
  bool: new _DynamicMapper.notEncodable(),
  Object: new _DynamicMapper.notEncodable(),
  Null: new _DynamicMapper.notEncodable()
};
...
class _MapDecoder {
  ...
  call(Object data, FieldDecoder fieldDecoder, 
    ...
    // Raw maps are normalized by the legacy model loaders after decoding.
    if (valueType == dynamic) return data;
  ...
  call(Object data, FieldDecoder fieldDecoder, 
    ...
    if (!clazz.isOriginalDeclaration) {
      ...
      if (clazz.isSubtypeOf(_listMirror)) {
        return new _DynamicMapper.notEncodable().decoder(data, 
            fieldDecoder, typeCodecs);
      }
      valueType = type;
    }
    
    // Raw lists are normalized by the legacy model loaders after decoding.
    if (valueType == dynamic) return data;
    var mapper = _getOrCreateMapper(valueType);
...
_DynamicMapper _getOrCreateMapper(Type type) {
  ...
  if (mapper == null) {
    ...
    if (isMap) {
      mapper = new _DynamicMapper.map();
      _cache[type] = mapper;
      return mapper;
    } else if(isList) {
      mapper = new _DynamicMapper.list();
      _cache[type] = mapper;
      return mapper;
    }
    ...
    var decodeChain = [];
    var encodeChain = [];
    var fields = <String, _FieldData>{};
    
    _buildChain(clazz, decodeChain, encodeChain, fields);
  
    if (fields.isEmpty) {
      
      mapper = new _DynamicMapper.notEncodable();
      ...
      var decoder = (data, fieldDecoder, typeCodecs, [fieldType]) {
        ...
        if (data is List) {
          return new _DynamicMapper.list().decoder(data, fieldDecoder, 
              typeCodecs, type);
        }
        var mirror;
        try {
