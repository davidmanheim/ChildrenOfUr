	...
	Future<Asset> load({Element statusElement : null, bool enableCrossOrigin : false})
	{
		...
		if (loaded == false)
		{
			...
			for (String ext in imageExtensions)
			{
				...
				{
					// Keep a statically typed reference here.  On current Dart-to-JS
					// output, calling `onLoad` through the dynamic `_asset` field
					// dispatches to the wrong legacy DOM member and fails at runtime.
					...
					image.onError.listen((Event err)
					{
						_asset = null;
