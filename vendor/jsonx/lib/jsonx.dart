...
decode(String text, {reviver(key, value), Type type}) {
  var json = jsonDecode(text, reviver: reviver);
...
final Map<Type, ConvertFunction> jsonToObjects = <Type, ConvertFunction> {
  DateTime: (dynamic input) => DateTime.parse(input as String)
};
  ...
  if (r == null) {
    ...
    if (m != _objectMirror) {
      ...
      m.declarations.forEach((k, v) {
        if (_isPublicField(v)) {
          VariableMirror field = v as VariableMirror;
          r[k] = new _Property(k, field.type, field.metadata);
        } else if (_isPublicGetter(v) && _hasSetter(m, v)) {
          MethodMirror getter = v as MethodMirror;
          r[k] = new _Property(k, getter.returnType, getter.metadata);
        }
      });
    }
  }
