...
class PoolImpl implements Pool {
  ...
  Future start() {
  final completer0 = new Completer<void>();
  scheduleMicrotask(() {
    try {
      _debug('start');
      join0() {
  ...
  final completer0 = new Completer<pg.Connection>();
  scheduleMicrotask(() {
    try {
      _debug('Connect.');
      join0() {
  ...
  final completer0 = new Completer<PooledConnectionImpl>();
  scheduleMicrotask(() {
    try {
      join0() {
        var stopwatch = new Stopwatch()
  ...
  Future<bool> _testConnection(
      ...
      Duration timeout, 
      Function onTimeout) {
  final completer0 = new Completer<bool>();
  scheduleMicrotask(() {
    try {
      bool ok;
      Exception exception;
