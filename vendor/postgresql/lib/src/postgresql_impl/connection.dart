...
class ConnectionImpl implements Connection {
  ...
  final Completer<ConnectionImpl> _connected = new Completer<ConnectionImpl>();
  ...
  final StreamController<Message> _messages = new StreamController<Message>.broadcast();
  ...
  Stream<Row> query(String sql, [values]) {
    try {
      if (values != null)
        sql = substitute(sql, values, (value, type) => _typeConverter.encode(value, type));
