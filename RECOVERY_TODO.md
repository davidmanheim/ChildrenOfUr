...
| active | Official visual-asset archive | Full `tinyspeck/glitch-items` archive fetched locally for inventory (~0.76 GB); source formats are FLA/SWF. Build deterministic browser sprite conversion and a manifest. |
...
| planned | Sound policy | Original world music is hard to recover from the released asset archive directly, but was preserved via YouTube playlist: https://www.youtube.com/playlist?list=PLN0GhXnhPcciUtcwFG5MfdT284einO0k1 . Next step: confirm per-track licensing/attribution, then fetch and bundle audio for deployment (or retain SoundCloud/YouTube as an optional fallback); browser gesture gating is expected either way. |
